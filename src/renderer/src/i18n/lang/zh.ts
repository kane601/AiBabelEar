export default {
  ...
  noti: {
    ...
    "pidInfo": "，字幕引擎进程 PID：",
    "empty": "模型路径为空",
    "emptyInfo": "本地模型路径为空，将自动使用默认目录 %APPDATA%/AiBabelEar/Vosk（Vosk）或 %APPDATA%/AiBabelEar/SOSV（SOSV）。请将对应模型下载到该目录后启动。",
    "stopped": "字幕引擎停止",
  ...
  engine: {
    ...
    "voskModelPathInfo": "Vosk 字幕引擎所需的模型文件夹路径。请将模型下载到 %APPDATA%/AiBabelEar/Vosk 目录（留空将自动使用该目录）。信息详情见项目用户手册。",
    "sosvModelPathInfo": "SOSV 字幕引擎所需的模型文件夹路径。请将模型下载到 %APPDATA%/AiBabelEar/SOSV 目录（留空将自动使用该目录）。信息详情见项目用户手册。",
